...
my (
    $token_text,     $last_type,        $last_text,
    $last_last_text, $last_word,        $current_mode,
    $indent_string,  $parser_pos,       $in_case,
...
my ( $opt_indent_level, $opt_indent_size, $opt_indent_character,
    $opt_preserve_newlines, $opt_space_after_anon_function );
        ...
        if ( $token_type eq 'TK_START_EXPR' ) {
            ...
            if ( $token_text eq '[' ) {
                ...
                if (   $current_mode eq '[EXPRESSION]'
                    || $current_mode eq '[INDENTED-EXPRESSION]' )
                {
           ...
           # and what does what.
            ...
            if (
                $last_type ne 'TK_END_BLOCK'
                && ( grep { lc($token_text) eq $_ }
                    ( 'else', 'catch', 'finally' ) )
              )
            {
                print_newline();
            }
            elsif ( ( grep { $token_text eq $_ } @line_starter )
                || $prefix eq 'NEWLINE' )
            {
                  ...
                  )
                {
                    ...
                    if ( ( grep { $token_text eq $_ } @line_starter )
                        && $last_text ne ')' )
                    {
                        print_newline();
                    }
            ...
            elsif ( $token_text eq '--' || $token_text eq '++' )
...
sub is_expression {
    my $mode = shift;
    return ( $mode eq '[EXPRESSION]'
          || $mode eq '[INDENTED-EXPRESSION]'
          || $mode eq '(EXPRESSION)' ) ? 1 : 0;
}
...
sub get_next_token {
    ...
    if ( $c eq '/' ) {
        ...
        if ( $input[$parser_pos] eq '/' ) {
            $comment = $c;
            while ($input[$parser_pos] ne "\x0d"
                && $input[$parser_pos] ne "\x0a" )
            {
                $comment .= $input[$parser_pos];
                $parser_pos++;
                if ( $parser_pos >= scalar @input ) {
    ...
    {    # regexp
        ...
        if ( $sep eq '/' ) {
            ...
            while ( $parser_pos < scalar @input
                && ( grep { $input[$parser_pos] eq $_ } @wordchar ) )
            {
                $resulting_string .= $input[$parser_pos];
                $parser_pos++;
            }
...
# http://mxr.mozilla.org/mozilla-central/source/js/src/jsscan.cpp around line 1935
        my $sharp = '#';
        if ( $parser_pos < scalar @input
            && ( grep { $input[$parser_pos] eq $_ } @digits ) )
        {
            do {
                $c = $input[$parser_pos];
                $sharp .= $c;
    ...
    if ( grep { $c eq $_ } @punct ) {
        while ( $parser_pos < scalar @input
            && ( grep { $c . $input[$parser_pos] eq $_ } @punct ) )
        {
            $c .= $input[$parser_pos];
            $parser_pos++;
...
package Mixi::ProductionJS;
use strict;
use warnings;

=usage
perl ~/JSPproduction.pl js_file need_make need_beautify
--js_file: full path;ex. /home/bkapps/gits/git-sdc/static/js/lib/prototype.js
--need_make: 0 1
--need_beaufity: 0 1

in vim, exectute the below command
:!perl ~/JSPproduction.pl %:p 1 1

or in vimrc, add the below line
autocmd BufWritePost *.js :!perl ~/JSProduction.pl %:p  1 1
=cut

my ( $buf_file, $need_make, $need_beautify ) = ( $ARGV[0], $ARGV[1], $ARGV[2] );

#my ($buf_file, $need_make, $need_beautify) =
#   ("/home/bkapps/gits/PLAN-360_comment_iine_phase2_2/static/js/mixi/touch/photo/view_photo.js",1,1);
#   ("/home/bkapps/gits/PLAN-360_comment_iine_phase2_4/static/js/mixi/touch/common/compat.js",1,1);

my ( @dir, $i, $len, $file, $mixi_root, $make_dir, $make_file );

sub to_vim { my $msg = shift; die($msg); }

to_vim($buf_file) unless $buf_file =~ /\.js$/;
...
$i   = $len;
while ( --$i ) {
    $mixi_root = join "/", @dir[ 0 .. $i - 1 ];
    last if -e "$mixi_root/mixi_root.txt"
}
to_vim("NOT MIXI PROJECT ?") unless $mixi_root;

if ($buf_file =~ m|^$mixi_root/static/js/mixi/touch/| ) {
    $make_dir  = $mixi_root."/static/js/mixi/touch/";
} else {
    $make_dir  = $mixi_root."/";
}
    $make_dir  = $mixi_root."/";
$make_file = $make_dir."Makefile";
$js_file   =~ s/$make_dir//g;

to_vim("File $make_file\n$js_file\n") unless -e $make_file;

#find production js in makefile
my $production;
@ARGV = ($make_file);
while (<>) {
    chomp;
    ...
    if (/^(.*): \\$/) {
        $production = $1;
        push @to_make_files, $production if $production eq $js_file;
    }
    if (/([^\\ ]+)/) {
        $file = $1;
        push @to_make_files, $production if $file eq $js_file;
    }
}
to_vim("to_make @to_make_files") unless @to_make_files;

#make production js
if ($need_make) {
    chdir($make_dir);
    system("make $_ ") for @to_make_files;
}

#beautify production js
if ($need_beautify) {
    my $FH;
    for (@to_make_files) {
        open $FH, "<", "$make_dir/$_" or to_vim("$make_dir/$_");
        my $minifer_code = do { local $/; <$FH> };
        close $FH;
        my $beautifier_code =
          JavaScriptBeautifier::js_beautify( $minifer_code,
            { indent_size => 2 } );
        open $FH, ">", "$make_dir/$_" or to_vim("$make_dir/$_");
        print $FH $beautifier_code;
        close $FH;
    }
}
1;
